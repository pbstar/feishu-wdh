...
> **提示**：首次使用需在扩展设置页面完成 API 配置。导出时，文档会先发送至你指定的大模型服务进行优化，处理完毕后再生成 ZIP 文件。
...
导出 ZIP 的**唯一主文件**为 `文档标题.md`：保留原文结构与段落顺序，逐段精简、去冗余，使表达更凝练，不丢失关键信息；必要时适度运用 Markdown 排版让文档更清晰易读。原文不直接导出，仅作为 AI 处理的输入。
...
- 导出时文档内容会发送给你自行配置的第三方大模型服务进行优化，该部分数据的处理受该服务商的隐私政策约束。
