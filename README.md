...
一键将当前 **飞书文档** 转换为 **Markdown**，图片一并下载并打包为 ZIP。可选接入大模型，额外生成 **AI 优化版** Markdown 与 **前端研发任务清单**。
...
- **AI 文档优化**（可选）：在导出原文之外，额外生成一份逐段精简的 AI 优化版 Markdown。
...
> **提示**：若启用了 [AI 文档优化](#ai-文档优化与前端研发任务总结可选) 或 [AI 前端研发任务总结](#ai-文档优化与前端研发任务总结可选)，需先在扩展设置页面中完成 API 配置。导出时，文档会先发送至你指定的大模型服务进行处理，处理完毕后再生成 ZIP 文件。

## AI 文档优化与前端研发任务总结（可选）

扩展默认**只导出原文** Markdown。如需 AI 加工，可在设置页面分别开启两个独立功能，两个功能共用同一套大模型 API 配置，**均默认关闭**。

### AI 文档优化

开启后，导出时**额外**生成一份 AI 优化版 Markdown（`文档标题-AI优化.md`）：保留原文结构与段落顺序，逐段精简、去冗余，使表达更凝练，不丢失关键信息。原文文件始终保留。

### AI 前端研发任务总结

开启后，导出时**额外**生成一份前端研发任务清单（`文档标题-任务清单.md`）：基于需求文档按功能模块梳理前端研发待开发任务，以任务列表（`- [ ]`）精炼呈现，可直接用于研发排期。
...
- 任一 AI 功能开启后，文档内容仅发送至你填写的**大模型服务商**，请自行确认其隐私政策。
- 提示词由扩展内置，无需手动编写。
...
- **唯一例外**：当你主动开启「AI 文档优化」或「AI 前端研发任务总结」后，文档内容会发送给你自行配置的第三方大模型服务，该部分数据的处理受该服务商的隐私政策约束。
